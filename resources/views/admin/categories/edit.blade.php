<form action="{{ route('category.update', $category->id) }}" method="POST">
    ...
    <el-dialog>
        <dialog id="editCategory{{ $category->id }}" aria-labelledby="dialog-title"
            ...
            <div tabindex="0"
                ...
                <el-dialog-panel
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="">
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 id="dialog-title" class="text-[20px] font-semibold text-black">Edit Category</h3>
                                <div class="mt-2">
                                    <label for="name" class="block text-sm font-medium text-[#000]">Name</label>
                                    <input type="text" name="name" id="name"
                                        value="{{ old('name', $category->name) }}"
                                        class="mt-1 block w-full p-2 border border-black rounded-md text-[#000] text-sm">
                                    ...
                                    @foreach ($types as $type)
                                        <div class="flex items-center space-x-2 py-1">
                                            <input type="radio" id="type_{{ $type->id }}" name="type_id"
                                                value="{{ $type->id }}"
                                                {{ $type->id == $category->type_id ? 'checked' : '' }}
                                                class="h-4 w-4 text-black border-gray-300 focus:ring-black">
                                            <label for="type_{{ $type->id }}" class="text-sm text-gray-700">
                                                {{ $type->type }}
                                            </label>
                                        </div>
                                    @endforeach
                    ...
                    <div class="bg-gray-700/25 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        ...
                        <button type="button" command="close" commandfor="editCategory{{ $category->id }}"
